...
function TickerBar({ market }: { market: string }) {
  // console.log('market is: ', market.split('_')[0]);
  return (
    <div className='flex h-[60px] shrink-0 space-x-4'>
      <div className='flex flex-row relative ml-2 -mr-4'>
        <img
          alt={`/${market.split('_')[0]}.webp`}
          loading='lazy'
          decoding='async'
          data-nimg='1'
          className='z-10 rounded-full h-6 w-6 mt-4 outline-baseBackgroundL1'
          src={`https://backpack.exchange/coins/${market.split('_')[0]}.svg`}
        />
        <img
          alt='USDC Logo'
          loading='lazy'
          ...
          src={`https://backpack.exchange/coins/${market.split('_')[1]}.svg`}
        />
      </div>
